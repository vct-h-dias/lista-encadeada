...
void Name_Sort(Lista *lista){
    ...
    node_aluno *before, *after, *aux, *i, *j;
    //criando dois nós que percoram o vetor e um auxiliar
    
    for(i = lista -> begin; i != NULL; i = i -> next ){
        
        before = i;
        if(before==lista->begin){

            aux=before;

        }
        if(aux->next!=before&&before!=lista->begin){
